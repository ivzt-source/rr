...
> **A ChatGPT Plus or Gemini Advanced subscription is a user-interface product. It does NOT
> include application programming interface access.** A script cannot log into your consumer
> subscription and drive the model. Programmatic multi-model work needs **application programming
> interface keys**, which are billed separately, per-token.
...
[OpenRouter](https://openrouter.ai) proxies most major models behind a single application
programming interface key and a single invoice. This is the **one-key simplicity** option — the
simplest way to get genuine cross-vendor diversity (GPT, Gemini, Claude, and more) without
juggling three billing accounts.
...
- Model identifiers look like `openai/gpt-5`, `google/gemini-2.5-pro`, `anthropic/claude-opus-4-8`.
...
Given the goal of "simplicity of one key to manage", this is usually the right pick.

## Option 3 — Direct vendor keys

Most control, least convenience — one key and one bill *per vendor*:

- **OpenAI's interface** (`OPENAI_API_KEY`) — pay-per-token, **no free tier**.
- **Google Gemini's interface** (`GEMINI_API_KEY`, via Google AI Studio) — pay-per-token, **has a
  free tier** (rate-limited, but $0). If you want a no-cost real-vendor voice, a direct Gemini key
  is the way.
- **Anthropic's interface** (`ANTHROPIC_API_KEY`) — pay-per-token.
...
base web address and key environment variable at the vendor (see the script header) — but
OpenRouter is the path of least resistance and what the script assumes by default.
